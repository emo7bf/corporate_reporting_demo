...
def color_for(idx): return PALETTE[idx % len(PALETTE)]

def escape(s): return html.escape(s or "")

def highlight_in_text(full_text: str, snippet: str, color: str) -> str:
    """Return HTML with the FIRST occurrence of snippet underlined & colored."""
    if not snippet:
        return f"<div class='panel'><div class='small'>No snippet to highlight.</div><div>{escape(full_text)}</div></div>"
    full = full_text
    snip = snippet.strip()
    i = full.find(snip)
    if i < 0:
        # fallback: just show the snippet in a container so reviewers still see the context wanted
        return f"<div class='panel'><legend>Target Snippet (not found verbatim)</legend><span class='hl' style='border-color:{color}'>{escape(snip)}</span></div>"
    before = escape(full[:i])
    hit = escape(full[i:i+len(snip)])
    after = escape(full[i+len(snip):])
...
def mini_diff(a: str, b: str) -> str:
    ...
    for token in ndiff(a.split(), b.split()):
        if token.startswith("+ "):
            out.append(f"<span style='background:rgba(18,183,106,.12);'>+{escape(token[2:])}</span>")
        elif token.startswith("- "):
            out.append(f"<span style='background:rgba(240,68,56,.12);'>-{escape(token[2:])}</span>")
        elif token.startswith("? "):
            # skip markers line
            continue
        else:
            out.append(escape(token[2:]))
    return "<div style='font-family:ui-monospace, SFMono-Regular, Menlo, monospace'>" + " ".join(out) + "</div>"

# Build a quick index: u.id -> index
...
with st.sidebar:
    ...
    search = st.text_input("Search updates (text/standard id)", "")
    ...
    # Navigator list
    selected_key = st.session_state.get("selected_id", filtered[0][1]["id"] if filtered else None)

    for i, upd in filtered:
        color = color_for(i)
        is_selected = (selected_key == upd["id"])
        border = f"border-color:{color};" if is_selected else ""
        st.markdown(
            f"<div class='update-chip' style='{border}' "
            f"onClick=\"window.parent.postMessage({{'type':'select','id':'{upd['id']}' }}, '*')\">"
            f"<span class='swatch' style='background:{color}'></span>"
            f"<span>{escape(upd['raw_request'])}</span>"
            f"</div>",
    ...
    for i, upd in filtered:
        color = color_for(i)
        for code in upd["standard_refs"]:
            st.markdown(f"<span class='codepill' style='border-color:{color};'>{escape(code)}</span>", unsafe_allow_html=True)

# JS to capture clicks on chips (simple workaround)
st.markdown("""
<script>
window.addEventListener("message", (e) => {
  if (e.data && e.data.type === "select") {
    const id = e.data.id;
    const patch = {'selected_id': id};
    window.parent.postMessage({isStreamlitMessage: true, type: "streamlit:setComponentValue", key: "selected_id", value: patch}, "*");
  }
});
</script>
""", unsafe_allow_html=True)

# Keep selected_id in session
sel_id_patch = st.session_state.get("selected_id")
if isinstance(sel_id_patch, dict) and "selected_id" in sel_id_patch:
    st.session_state["selected_id"] = sel_id_patch["selected_id"]
elif "selected_id" not in st.session_state and filtered:
    st.session_state["selected_id"] = filtered[0][1]["id"]

# Resolve selected update
sel_id = st.session_state.get("selected_id")
sel_idx = id_to_idx.get(sel_id, filtered[0][0] if filtered else 0) if sel_id else (filtered[0][0] if filtered else 0)
selected = transformed_updates[sel_idx] if filtered else None
sel_color = color_for(sel_idx) if filtered else "#999"
...
with colA:
    ...
    st.caption("Hover/Click an update to see the precise passage.")
    if selected:
        st.markdown(highlight_in_text(original_text_2023, selected["original_text_to_highlight"], sel_color), unsafe_allow_html=True)
    else:
        st.info("No updates match your current filters.")

with colB:
    st.subheader("Updated (2024)")
    st.caption("The passage reflecting the change.")
    if selected:
        st.markdown(highlight_in_text(outputted_text_2024, selected["new_text_to_highlight"], sel_color), unsafe_allow_html=True)
    else:
        st.info("No updates match your current filters.")

# Standards drawer + diff
st.markdown("### Standards & Details")
with st.expander("Show standards for the selected update", expanded=True):
    if selected:
        st.markdown(f"<div class='panel'>"
                    f"<legend>Linked Standards</legend>"
                    + "".join(
                        f"<div class='codepill' style='border-color:{sel_color}; display:inline-block'>{escape(code)}</div>"
                        for code in selected["standard_refs"]
                    )
                    + "<div style='height:.5rem'></div>"
                    + "".join(
                        f"<div class='small'><b>{escape(code)}</b>: {escape(standards_index.get(code, {}).get('clause',''))}</div>"
                        for code in selected["standard_refs"]
                    )
...
if show_diff and selected:
    old_snip = selected["original_text_to_highlight"] or ""
    new_snip = selected["new_text_to_highlight"] or ""
...
st.download_button("Download selected update bundle (JSON)", data=json.dumps(bundle, ensure_ascii=False, indent=2), file_name=f"{sel_id or 'update'}_bundle.json")

# Queue controls (quick review flow)
st.markdown("---")
col_prev, col_next = st.columns([1,1])
with col_prev:
    if st.button("◀ Previous"):
        if filtered:
            i = [i for i, u in filtered].index(sel_idx) if selected else 0
            new_i = max(0, i-1)
            st.session_state["selected_id"] = filtered[new_i][1]["id"]
            st.rerun()
with col_next:
    if st.button("Next ▶"):
        if filtered:
            i = [i for i, u in filtered].index(sel_idx) if selected else 0
            new_i = min(len(filtered)-1, i+1)
            st.session_state["selected_id"] = filtered[new_i][1]["id"]
            st.rerun()
